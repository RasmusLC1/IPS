...
(* (except as comments), so no particular restrictions. *)
...
let rec compileExp  (e      : TypedExp)
  ...
  *)
  ...
  | Divide (e1, e2, pos) ->
      let t1 = newReg "div1_L"
      let t2 = newReg "div2_R"
      let divzero = newLab "divzero"
      let code1 = compileExp e1 vtable t1
      let code2 = compileExp e2 vtable t2
      code1 @ [LI (place, 0)
        ; BEQ (t1, place, divzero)
        ]    
      @code2 @ [BEQ (t2, place, divzero)
        ; DIV (place,t1,t2)
        ; LABEL divzero]
  ...
  *)
  ...
  | And (e1, e2, pos) ->
    let t1 = newReg "and_L"
    let t2 = newReg "and_R"
    let falselab = newLab "false"
    let code1 = compileExp e1 vtable t1
    let code2 = compileExp e2 vtable t2
    code1 @ [LI (place, 0)
    ; BEQ (t1, place, falselab)
    ]    
    @code2 @ [BEQ (t2, place, falselab)
    ; LI (place, 1)
    ; LABEL falselab ]
  ...
  | Or (e1, e2, pos) ->
    let t1 = newReg "or_L"
    let t2 = newReg "or_R"
    let truelab = newLab "true"
    let code1 = compileExp e1 vtable t1
    let code2 = compileExp e2 vtable t2
    code1 @ [LI (place, 1)
    ; BEQ (t1, place, truelab)
    ]    
    @code2 @ [BEQ (t2, place, truelab)
    ; LI (place, 1)
    ; LABEL truelab ]
  ...
  *)
  | Replicate (n, a, tp, pos) ->
      let n_reg = newReg "n_reg"
      let i_reg = newReg "i_reg"
      let a_reg = newReg "a_reg"
      let res_reg  = newReg "arr"   (* address of array *)
      let arr_code = compileExp n vtable n_reg
      let n_code = compileExp a vtable a_reg
      let alloc_code = dynalloc (n_reg, place, tp)
      let loop_beg = newLab "loop_beg"
      let loop_end = newLab "loop_end"

    
      let array_creation_loop =
              [ MV(i_reg, Rzero)
              ; ADDI(res_reg, place, 4)          // add 4 to start to reach first element
              ; LABEL(loop_beg)                  // Generate label
              ; BEQ(i_reg, n_reg, loop_end)      // check if i == n
              ; ADDI(i_reg, i_reg, 1)            // i++
              ]
      let replicate_loop = 
          match getElemSize tp with
            | ESByte  -> [ SB   (a_reg, res_reg, 0) // move a_reg to to arr_reg 0
                      ; ADDI (res_reg, res_reg, 1) // arr_reg byte
                      ]
            | ESWord -> [ SW   (a_reg, res_reg, 0) // move a_reg to to arr_reg 0
                      ; ADDI (res_reg, res_reg, 4) // arr_reg word
                      ]
      arr_code
       @ n_code
       @ alloc_code
  ...
  (* DONE project task 2: see also the comment to replicate.
  ...
  *)
  ...
  | Filter (func, arr_exp, tp, pos) ->
      let arr_reg  = newReg "arr_reg" 
      let temp_reg  = newReg "temp_reg"
      let bool_reg  = newReg "temp_reg"
      let res_reg  = newReg "res_reg"
      let res_addr_reg  = newReg "res_reg"
      let addr_reg  = newReg "acc_reg"
      let i_reg    = newReg "i_reg"   
      let j_reg    = newReg "j_reg"
      let length_reg = newReg "size_reg"
      let alloc_code = dynalloc (length_reg, place, tp)
      let arr_code = compileExp arr_exp vtable arr_reg
      let loop_beg = newLab "loop_beg"
      let loop_end = newLab "loop_end"
      let filter_false = newLab "filter_false"

      let length = [ LW (length_reg, arr_reg, 0) ]
      let init_regs = [ ADDI (addr_reg, place, 4)
                      ; MV (i_reg, Rzero)
                      ; ADDI (temp_reg, arr_reg, 4)
                      ; MV (j_reg, Rzero)
                      ]
      ...
      let loop =
             [ LABEL (loop_beg)
             ; BGE (i_reg, length_reg, loop_end)
             ; Load size (res_addr_reg, temp_reg, 0)
             ; Load size (bool_reg, temp_reg, 0)
             ; ADDI (temp_reg, temp_reg, elemSizeToInt size)
             ]

             @ applyFunArg(func, [bool_reg], vtable, res_reg, pos)

             @ [ BEQ (res_reg, Rzero, filter_false)
             ; Store size (res_addr_reg, addr_reg, 0)
             ; ADDI (j_reg, j_reg, 1)
      ...
      let store = 
          match getElemSize tp with
            | ESByte  -> [ SB   (j_reg, place, 0)]
            | ESWord -> [ SW   (j_reg, place, 0)]    
  ...
  *)
  ...
  | Scan (func, acc_exp, arr_exp, tp, pos) ->
      let arr_reg  = newReg "arr_reg" 
      let temp_reg  = newReg "temp_reg"
      let res_reg  = newReg "res_reg"
      ...
      let start = [ LW(length_reg, arr_reg, 0) ]
                   @dynalloc (length_reg, place, tp)
      ...
      let loop =
              [ LABEL(loop_beg)
              ; SUB(temp_reg, i_reg, length_reg)
              ; ADDI(i_reg, i_reg, 1)
              ; BGEZ(temp_reg, loop_end)
              ]
      //Store arr in temp array
      let store_temp =
              match getElemSize tp with
                | ESByte  -> [ LB   (temp_reg, arr_reg, 0)
                          ; ADDI (arr_reg, arr_reg, 1)
                          ]
                | ESWord -> [ LW   (temp_reg, arr_reg, 0)
                          ; ADDI (arr_reg, arr_reg, 4)
                          ]
      //Store temp in result array
      let store_arr =
              match getElemSize tp with
                | ESByte ->  [ SB   (acc_reg, res_reg, 0)
                          ; ADDI (res_reg, res_reg, 1)
                          ]
                | ESWord -> [ SW   (acc_reg, res_reg, 0)
                          ; ADDI  (res_reg, res_reg, 4)
                          ]
      let apply_code =
              applyFunArg(func, [acc_reg; temp_reg], vtable, acc_reg, pos)
...
let rec stackSave (currentReg  : int)
    ...
    else let noffset = offset-4 (* adust offset *)
...
let compile (funs : TypedProg) : Instruction list =
  let () = stringTable := [("s.true","true"); ("s.false","false")]
  ...
  let main_prog =
    ...
    @ [LA (Rhp, "d.heap")]
      (* call main function (and stop after returning) *)
    @ [JAL ("f.main",[])]
    @ (* stop code *)
      [LABEL "l.stop";
       LI (Rsn, sysExit);
