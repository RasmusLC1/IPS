        ...
        | Let (Dec (name, e, decpos), body, pos) ->
            ...
            match e' with
                | Var (s, pos) ->
                    ...
                    *)
                    let vtable' = SymTab.bind name (VarProp s) vtable
                    let body' = copyConstPropFoldExp vtable' body
                    Let (Dec (name, Var(s, pos), decpos), body', pos)
                ...
                | Constant (v, pos) ->
                    (* TODO project task 3:
                        Hint: I have discovered a constant-copy statement `let x = 5`.
                              ...
                              and optimize the `body` of the let.
                    *)
                    let vtable' = SymTab.bind name (ConstProp v) vtable
                    let body' = copyConstPropFoldExp vtable' body
                    Let (Dec (name, Constant(v, pos), decpos), body', pos)

                | Let (Dec (name2, e2, decpos2), e1, pos2) ->
                    (* TODO project task 3:
                    ...
                    *)
                    let restructuredExp = Let (Dec (name2, e2, decpos2), Let (Dec (name, e1, decpos2), body, pos2), pos) in
                    copyConstPropFoldExp vtable restructuredExp
