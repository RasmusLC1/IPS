...
let rec evalExp (e : UntypedExp, vtab : VarTable, ftab : FunTable) : Value =
  ...
  *)
  ...
  | Divide (e1, e2, pos) ->
        ...
        match (res1, res2) with
          | (IntVal n1, IntVal n2) ->
                                  if n1 = 0 || n2 = 0 then
                                    reportWrongType ("Division by 0") Int res1 (expPos e2)
                                  else
                                    IntVal (n1/n2)
          | (IntVal _, _) -> reportWrongType "right operand of /" Int res2 (expPos e2)
          | (_, _) -> reportWrongType "left operand of /" Int res1 (expPos e1)
  ...
  | And (e1, e2, pos) ->
        ...
        match res1 with 
          ...
          | _ -> let res2   = evalExp(e2, vtab, ftab)
                 match res2 with
                   | (BoolVal false) -> BoolVal false
                   | _ -> BoolVal true
  ...
  | Not(e1, pos) ->
        ...
        match res1 with
          ...
          | (_) -> reportWrongType "left operand of not" Bool res1 (expPos e1)
  ...
  (* DONE project task 2: `replicate(n, a)`
  ...
  *)
  | Replicate (n, a, tp, pos) ->
        let lenth = evalExp(n, vtab, ftab)
        let copyVal = evalExp(a, vtab, ftab)
        match lenth with
          | IntVal i ->
                      if i >= 0 then
                        ArrayVal((List.replicate i copyVal), valueType(copyVal))
                      else
                        failwith "intval negative Replicate"
          | _ -> failwith "intval negative Replicate not int"
  ...
  *)
  | Filter (p, arrexp, tp, pos) ->
    let p_type = rtpFunArg p ftab pos
    match p_type with
      | Bool -> ()
      | other -> failwith "filter error"
    let arr = evalExp(arrexp, vtab, ftab)
    match arr with
    | ArrayVal (lst, tp1) -> ArrayVal (List.filter (
      fun x -> match evalFunArg (p, vtab, ftab, pos, [x]) with
                | BoolVal b -> b
                | _ -> failwith "filter erorr"
                )lst, tp1)
  ...
  *)
  | Scan (farg, ne, arrexp, tp, pos) ->
        ...
        match arr with
          | ArrayVal (lst,tp1) ->
               ArrayVal(List.tail (List.scan(fun acc x -> evalFunArg (farg, vtab, ftab, pos, [acc;x])) nel lst), Array farg_ret_type)
