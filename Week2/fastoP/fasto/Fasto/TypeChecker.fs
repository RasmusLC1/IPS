...
and checkExp  (ftab : FunTable)
    ...
    *)
    ...
    | Not (e1, pos) ->
        let (e1_type, e1_typeExp) = checkExp ftab vtab (e1)
        if e1_type = Bool then 
          Bool, Not (e1_typeExp, pos)
        else
          failwith "Not correct type for NOT"
            
    | Negate (e1, pos) ->
        let (e1_type, e1_typeExp) = checkExp ftab vtab (e1)
        if e1_type = Int then 
          (Int, Negate (e1_typeExp, pos))
        else
          failwith "Not correct type for NOT"
    ...
    *)
    | Replicate (n_exp, a_exp, _, pos) ->
        ...
        let  (a_type, a_exp2) = checkExp ftab vtab a_exp
        if n_type = Int then
          (Array a_type, Replicate(n_dec, a_exp2, a_type, pos))
        else
          reportTypeWrongKind "Replicate input is not int" "input" n_type pos

    (* DONE project task 2: Hint for `filter(f, arr)`
    ...
    *)
    | Scan (f, e_exp, arr_exp, _, pos) ->
        ...
        let elem_type =
            match arr_type with
              | Array t -> t
              | _ -> reportTypeWrongKind "second argument of Scam" "array" arr_type pos

        let (f', farg_type) =
            match checkFunArg ftab vtab pos f with
              | (f', res, [a1; a2]) ->
                  if a1 = a2 && a2 = res then
                    (f', res)
                  else
                    reportTypesDifferent "arguments of Scan " a1 a2 pos
              | _ ->failwith "Scan fail"

        if elem_type = farg_type && elem_type = e_type then
